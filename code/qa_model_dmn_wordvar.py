...
class EpisodicMemory():
    ...
    def _attention(self, q, m, s):
        ...
        z = tf.contrib.layers.fully_connected(tf.contrib.layers.fully_connected(z, self.ahl_units , activation_fn=tf.nn.tanh), \
...
#         self.encoder_size = encoder_out_dim/2
    ...
    def loop_fn_transition(self, time, previous_output, previous_state, previous_loop_state):
        ...
        def get_next_input():
            ...
            _output_logits = tf.expand_dims(output_logits, axis = 2) #batch, cont_length, 1
            _next_input = tf.reduce_sum(_output_logits * self.context_embed, axis = 1) #batch, 2*encoder_size
#             prediction = tf.argmax(output_logits, axis=1, output_type = tf.int32) #batch
#             be careful while using logits to make predictions. if all logits are negative then the prediction comes
#             out of paddings which is not what the prediction would be out of probabilities*masks which will need to be done
#             argmax introduces non differentiability trying to avoid it
...
# tf.cond is currently throwing invalidargument error. Resolution: unknown. mostly it is a bug
        ...
        previous_loop_state = previous_loop_state.write(time - 1, output_logits)
...
class QASystem(object):
    def __init__(self, encoder, decoder, episodic_mem_module, pretrained_embeddings, config, train_flag = True):
        ...
        self.pretrained_embeddings_vars  = tf.Variable(pretrained_embeddings, dtype = tf.float32)
        self.pretrained_embeddings_const = tf.constant(pretrained_embeddings, dtype = tf.float32)
        ...
        self.q_id = tf.placeholder(tf.int32, shape = [None, None]) #batch_size x question_length
        self.c_words_id = tf.placeholder(tf.int32, shape = [None, None]) #batch_size x sent_max_length
        self.c_sent_masks = tf.placeholder(tf.int32, shape = [None, None]) #batch, context_length
    ...
    def setup_system(self):
        ...
        with tf.variable_scope('collecting_sentences'):
            self.c, self.c_masks = self.collect_sentences(encoder_c_words, self.c_sent_masks)
...
#             encoder_c, encoder_c_final_state = self.encoder.encode(self.c, self.c_masks)
        with tf.variable_scope("episodic_memory"):
            ...
            episode_mem = self.episodic_mem_module.knowledge_rep(self.c, encoder_q_final_state, seq_length) #batch, 2*encoder_size
        with tf.variable_scope("decoder_call"):
            ...
            logits = out.stack() #2, batch, max_time_word
            self.logits_s = self.masked_exp_logits(logits[0]) #batch, max_time_words
            self.logits_e = self.masked_exp_logits(logits[1]) #batch, max_time_word
            self.prediction_s = tf.argmax(self.logits_s, axis = 1) #batch
            self.prediction_e = tf.argmax(self.logits_e, axis = 1) #batch
            
            
    
    def masked_exp_logits(self, logits):
        ...
        return exp_logits_masked
        
        
        
    def _loss(self,exp_logits_masked, labels):
        B, L = tf.shape(exp_logits_masked)[0], tf.shape(exp_logits_masked)[1]
    ...
    def setup_loss(self):
        ...
        with vs.variable_scope("loss"): #check this logprobs and probs
    ...
    def embed_lookup(self):
        q_var = tf.nn.embedding_lookup(self.pretrained_embeddings_vars, self.q_id) #batch,q_max_len,embed_dim
        q_const = tf.nn.embedding_lookup(self.pretrained_embeddings_const, self.q_id) #batch, q_max_len, embed_dim
        c_words_var = tf.nn.embedding_lookup(self.pretrained_embeddings_vars, self.c_words_id) #batch, context_len, embed_dim
        c_words_const = tf.nn.embedding_lookup(self.pretrained_embeddings_const, self.c_words_id) #batch, context_len, embed_dim
        self.q = tf.concat([q_var, q_const], axis = 2)
        self.c_words = tf.concat([c_words_var, c_words_const], axis = 2)
        
        
    def collect_sentences(self, word_states, ids):
        '''collects sentence representation for word_states based on ids
        params:
        ids:boolean[batch, max_words_len]
        word_states: [batch, max_words_len, 2*encoder_size]'''
        def enhancer(last_output, current_input):
            start, _, _ = last_output
            end = start + current_input
            pad_in = tf.tile(pad, [max_length - current_input, 1])
            new_t = tf.concat([states[start:end], pad_in], axis = 0)
            new_m = tf.concat([tf.ones([current_input], tf.int32), tf.zeros([max_length - current_input], tf.int32)], axis = 0)
            start = end
            return (start, new_t, new_m)
        lengths = tf.reduce_sum(ids, axis = 1)
        max_length = tf.reduce_max(lengths)
        states = tf.boolean_mask(word_states, ids) #len,
        B = word_states.get_shape().as_list()
        dim_size = word_states.get_shape().as_list()[-1]
        pad = tf.constant(0, dtype = tf.float32, shape = [1, dim_size])
        start = 0
        t_s = states[start:lengths[0]]
        pad_t = tf.tile(pad, [max_length - lengths[0],1])
        t = tf.concat([t_s, pad_t], axis = 0)
        masks  =  tf.concat([tf.ones([lengths[0]], tf.int32), tf.zeros([max_length - lengths[0]], tf.int32)], axis = 0)
        result = tf.scan(enhancer, lengths[1:], initializer=(lengths[0], t, masks))
        c = tf.concat([[t], result[1]], axis = 0)
        mask = tf.concat([[masks], result[2]], axis = 0)
        return c, mask
...
#         indices = np.sort(indices)
    ...
    def feed_dict(self, dataset_feed):
        '''dataset_feed: ([q_pad, c_pad, , c_sent_masks_pad, a (just indices)], dat_pad_masks)'''
        input_feed = {}
        padded, padded_masks = dataset_feed
        batch_size = len(padded[0])
        input_feed[self.q_id], input_feed[self.c_words_id], input_feed[self.c_sent_masks], input_feed[self.a] = padded
        input_feed[self.q_masks], input_feed[self.c_words_masks], _ = padded_masks
    ...
    def prepare_feed_input(self, dataset_minix):
        ...
        Args:
          Inputs:
                dataset_minix: [q, c, a, c_sent_masks]
          Return:
                dataset_i: ([q_pad, c_pad, , c_sent_masks_pad, a (just indices)], dat_pad_masks)'''
        q,c,a,c_period_masks = dataset_minix
        pad_input = [q, c, c_period_masks]
        padded_input, padded_masks = self.pad(pad_input)
        padded_input.append(a)
        return padded_input, padded_masks
    ...
    def train(self, session, dataset, epochs, period_location, train_dir):
        ...
        dataset_train = [[[int(x) for x in t.split()] for t in j] for j in dataset_train_raw]
        dataset_val = [[[int(x) for x in t.split()] for t in j] for j in dataset_val_raw]
        
#         print(period_location, dataset_train[1])
        c_train = [j + period_location if j[-1] != period_location[0] else j for j in dataset_train[1]]
        c_val = [j + period_location if j[-1] != period_location[0] else j for j in dataset_val[1]]
        period_mask_train = [[1 if j == period_location[0] else 0 for j in t] for t in c_train]
        period_mask_val = [[1 if j == period_location[0] else 0 for j in t] for t in c_val]
        dataset_train[1] = c_train
        dataset_val[1] = c_val
                 
        dataset_train.append(period_mask_train)
        dataset_val.append(period_mask_val)
        
        #dataset sent in from train would be of the format [q, c, a, c_period_mask]
