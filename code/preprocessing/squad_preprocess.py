...
def invert_map(answer_map):
    return {v[1]: [v[0], k] for k, v in list(answer_map.items())}
...
def read_write_dataset(dataset, tier, prefix):
    ...
    with open(os.path.join(prefix, tier +'.context'), 'w', encoding = 'utf8') as context_file,  \
         open(os.path.join(prefix, tier +'.question'), 'w', encoding = 'utf8') as question_file,\
         open(os.path.join(prefix, tier +'.answer'), 'w', encoding = 'utf8') as text_file, \
         open(os.path.join(prefix, tier +'.span'), 'w', encoding = 'utf8') as span_file:
...
#                             context_tokens1 = [i.decode('utf8') for i in context_tokens]
#                             question_tokens1 = [i.decode('utf8') for i in question_tokens]
#                             text_tokens1 = [i.decode('utf8') for i in text_tokens]
...
def split_tier(prefix, train_percentage = 0.9, shuffle=False):
    ...
    with open(context_filename, encoding = 'utf8') as current_file:
        num_lines = sum(1 for line in current_file)
    # Get indices and split into two files
    indices_dev = [i for i in range(num_lines)][int(num_lines * train_percentage)::]
    if shuffle:
        np.random.shuffle(indices_dev)
        print("Shuffling...")
    save_files(prefix, 'val', indices_dev)
    indices_train = [i for i in range(num_lines)][:int(num_lines * train_percentage)]
    if shuffle:
